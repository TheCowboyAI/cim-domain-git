...
use cim_domain_git::{
    commands::*,
    events::{GitDomainEvent, FileChangeType},
    handlers::{RepositoryCommandHandler, extract_dependency_graph},
};
...
async fn test_analyze_real_repository() {
    ...
    let result = handler.analyze_repository_at_path(temp_dir.path().to_str().unwrap()).await;
    
    assert!(result.is_ok());
    let (repo_id, events) = result.unwrap();
    
    // Should have repository analyzed event
    assert!(events.iter().any(|e| matches!(e, GitDomainEvent::RepositoryAnalyzed(_))));
    
    // Should have branch events
    let branch_events: Vec<_> = events.iter()
        .filter(|e| matches!(e, GitDomainEvent::BranchCreated(_)))
        .collect();
    assert!(!branch_events.is_empty());
    
    // Should have commit events
    let commit_events: Vec<_> = events.iter()
...
async fn test_extract_commit_graph_from_real_repo() {
    ...
    let (repo_id, _) = handler.analyze_repository_at_path(temp_dir.path().to_str().unwrap())
        .await
        .unwrap();
...
async fn test_extract_dependency_graph_from_real_repo() {
    ...
    let events = extract_dependency_graph(cmd, &git_repo)
        .await
        .unwrap();
...
async fn test_file_change_tracking() {
    ...
    // Analyze repository
    let handler = RepositoryCommandHandler::new();
    let (_repo_id, events) = handler.analyze_repository_at_path(temp_dir.path().to_str().unwrap())
        .await
        .unwrap();
    ...
    let update_commit = commit_events.iter()
    ...
    assert_eq!(update_commit.files_changed[0].change_type, FileChangeType::Modified);
}
...
async fn test_branch_operations() {
    ...
    // Analyze repository
    let (_repo_id, initial_events) = handler.analyze_repository_at_path(temp_dir.path().to_str().unwrap())
        .await
        .unwrap();
    
    // Count initial branches
    let initial_branch_count = initial_events.iter()
        .filter(|e| matches!(e, GitDomainEvent::BranchCreated(_)))
        .count();
    ...
    let (_repo_id2, new_events) = handler.analyze_repository_at_path(temp_dir.path().to_str().unwrap())
        .await
        .unwrap();
    
    let new_branch_count = new_events.iter()
        .filter(|e| matches!(e, GitDomainEvent::BranchCreated(_)))
        .count();
