...
impl NatsClient {
    ...
    pub async fn connect(config: NatsConfig) -> Result<Self> {
        ...
        if config.tls.is_some() {
            debug!("TLS configuration provided");
            // Note: In async-nats 0.42, TLS is automatically used when connecting to a tls:// or wss:// URL
            // Certificate configuration would need to be done through rustls directly
            if config.url.starts_with("nats://") {
                warn!("TLS configuration provided but using non-TLS URL. Consider using tls://");
            }
        }
    ...
    async fn ensure_streams(&self, jetstream: &async_nats::jetstream::Context) -> Result<()> {
        use async_nats::jetstream::{
            stream::Config as StreamConfig, stream::StorageType as JsStorageType,
        };
        ...
        match jetstream.get_or_create_stream(command_stream_config).await {
            Ok(_) => info!(
                ...
                self.config.jetstream.command_stream
            ),
            Err(e) => {
                return Err(NatsError::Other(format!(
